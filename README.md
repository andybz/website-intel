...
- Basic HTTP 404 tracking — consolidated into a single issue per site (not grouped per-URL anymore, see hardening note below) with a "Most Requested Paths" breakdown
...
- Data retention (section 54): hourly aggregates (`issue_hourly_counts`, `pageview_hourly_counts`) are now pruned after 1 year, and expired sessions/pairing-tokens/password-reset-tokens are cleaned up too — no cron infrastructure exists in this app, so cleanup runs opportunistically (very low probability check) from the busiest ingestion endpoints (`src/lib/server/retention.ts`), mirroring the WordPress plugin's own opportunistic-heartbeat pattern
...
**Phase 3 — Advanced Intelligence: 🚧 Started**
...
- Vendored the open-source ["Plugin Update Checker"](https://github.com/YahnisElsts/plugin-update-checker) library (MIT, v5.7) into `wordpress-plugin/causetrail-monitor/vendor/` - it hooks into WordPress's native Dashboard → Updates / "Update available" UI, just pointed at a URL we control instead of the WordPress.org API
- `scripts/build-plugin-package.ts` runs as part of `npm run build` (every deploy): zips the current `wordpress-plugin/causetrail-monitor/` directory and writes a JSON manifest (name/version/download_url, parsed from the plugin's own `Version:` header) into `static/downloads/`, so both are served as plain static files at `https://monitor.andybz.com/downloads/causetrail-monitor.{zip,json}` - no new runtime server code, no auth needed (the plugin is useless without a real pairing key regardless of who can download its source)
...
- **One remaining manual step, unavoidable:** every already-connected site still needs ONE final manual re-upload of the plugin (to v0.9.0+, which also includes the folder rename below) to gain the auto-updater itself - a plugin can't retroactively grant itself update-checking capability. Every version after that updates natively through wp-admin.
...
- ~~**Ask Your Website** (section 31)~~ — ✅ built (see Phase 3 above). Future enhancement: multi-turn conversation memory (current version is single-question, no history persisted).
...
- Full UI/design pass built around the CauseTrail brand identity (navy `#0b1b33` / blue `#1677f2` / teal `#25c4c8` gradient, "connected trail of nodes resolving to a root cause" concept from the icon): dark navy header with a gradient underline, a navy gradient-glow backdrop on all auth pages (login/forgot-password/reset-password), a new `HealthRing` component (circular gauge using the brand gradient, replacing plain health score text on the dashboard and site Overview page), gradient primary buttons app-wide (`.btn-primary` in `layout.css`), badges restyled with small outlined-circle dot indicators, a gradient active-tab indicator on site detail pages, and a connected-dot "trail" timeline visual on the Activity tab (red dots for issues, teal for changes, gradient connector line). Design tokens live in `src/routes/layout.css` under `@theme`.
- Site Overview redesign: hero section pairing a live homepage screenshot (WordPress.com's free `mshots` service, no API key) with the health ring and big-number stat cards, a 7-day traffic trend chart pulled onto the page itself, and a prominent pulsing banner when health is critical, linking straight to the worst issue.
- Fixed real-world 404 noise: on busy client sites, bot/vulnerability scanners probing hundreds of distinct nonexistent paths (`.env`, `wp-config.php.bak`, random plugin slugs, etc.) were each creating their OWN issue (since the URL was part of the fingerprinted message), flooding the Issues list. Reworked `http_404` to consolidate into a single issue per site (same pattern as `failed_login`'s existing throttled grouping) — the plugin (v0.5.0) now tracks a rolling tally of the top 10 most-requested 404 paths itself and reports it as metadata, shown on the issue detail page under "Most Requested Paths" (directly answering "is this a bot"). A one-off migration script (`scripts/consolidate-404-issues.ts`, dry-run by default, `--apply` to execute) merges any already-scattered 404 issues from before this fix.
